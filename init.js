...
module.exports = function (server, options) {
    ...
    function tryCreate() {
        if (tryCount > 5) {
            console.log('KAAE: Failed creating Indices Mapping');
            return;
        }
    ...
    function createKaaeAlarmIndex() {
        ...
        client.indices.exists({
            ...
            if (exists === true) {
                console.log('Kaae Alarm index exists');
                return;
            }
    ...
    function tryAlarmCreate() {
        if (tryCount > 5) {
            console.log('KAAE: Failed creating Alarm Indices Mapping');
            return;
        }
        ...
        getCount().then(function (resp) {
            getWatcher(resp.count).then(function (resp) {
                _.each(resp.hits.hits, function (hit) {
                    ...
                    Schedule[hit._id] = later.setInterval(function () {
