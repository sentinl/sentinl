      ...
      _.each(resp.hits.hits, function(hit){
        ...
        if (Schedule[hit._id]) {
          ...
          function watching(task,interval) {
            server.log(['status', 'info', 'KaaE'], 'Executing watch: '+task._id+' Next Round of '+task._id+' at '+later.schedule(interval).next(2) );
        ...
        function reporting(task,interval) {
          server.log(['status', 'info', 'KaaE'], 'Executing report: '+task._id+' Next round at '+later.schedule(interval).next(2) );
